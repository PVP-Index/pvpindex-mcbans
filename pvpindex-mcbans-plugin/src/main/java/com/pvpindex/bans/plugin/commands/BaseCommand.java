...
public abstract class BaseCommand {
    ...
    public boolean run(final MCBans plugin, final CommandSender sender, final String cmd, final String[] preArgs) {
        ...
        if (banning && args.size() > 0){
            ...
            // get targetIP and UUID if player is online
            final Player targetPlayer = MCBans.getPlayer(plugin, target);
            if (targetPlayer != null && targetPlayer.isOnline()){
                targetUUID = targetPlayer.getUniqueId().toString().replaceAll("-", "");
            ...
            if (targetUUID == null && target != null) {
                ...
                if (offlinePlayer.hasPlayedBefore() || offlinePlayer.isOnline()) {
                    targetUUID = offlinePlayer.getUniqueId().toString().replaceAll("-", "");
                }
            }
